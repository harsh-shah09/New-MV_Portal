...
import { createAsset, getProducts, getNextInternalSerialNumber } from '../actions';
    ...
    const handleFinish = async (values: any) => {
        ...
        try {
            await createAsset({
                ...
                Internal_Serial_Number__c: values.Internal_Serial_Number__c,
            });
    ...
    const handleProductChange = async (productId: string) => {
        ...
        if (prod) {
            form.setFieldsValue({
                ...
                Internal_Serial_Number__c: undefined
            });

            setIsGeneratingSerial(true);
            try {
                const nextSerial = await getNextInternalSerialNumber(prod.AMS_Category__c);
                form.setFieldsValue({
                    Internal_Serial_Number__c: nextSerial
                });
            } catch (e) {
                console.error("Failed to generate internal serial number", e);
                showToast.error("Generation Failed", { description: "Failed to auto-generate internal serial number." });
            } finally {
                setIsGeneratingSerial(false);
            }
        } else {
            form.setFieldsValue({
                AMS_Category__c: undefined,
                Internal_Serial_Number__c: undefined
            });
        }
    }
        ...
        >
            ...
            >
                ...
                <div className="grid grid-cols-3 gap-4">
                    ...
                    <Form.Item name="Internal_Serial_Number__c" label="Internal Serial Number">
                        <Input
                            ...
                            readOnly
                            suffix={isGeneratingSerial ? <LoadingOutlined spin className="text-blue-500" /> : null}
                        />
                    </Form.Item>
                    <Form.Item
                ...
                <div className="flex justify-end gap-2 pt-4">
                    ...
                    <Button type="primary" htmlType="submit" loading={loading || isGeneratingSerial}>
                        Register Asset
                    </Button>
                </div>
            </Form>
